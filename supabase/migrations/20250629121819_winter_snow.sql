/*
  # Fix Database Sync and Auto-Profile Creation

  1. New Functions
    - `handle_new_user()` - Automatically creates profiles when users sign up
  
  2. New Triggers
    - `on_auth_user_created` - Triggers profile creation on user registration
  
  3. Demo Data
    - Creates demo users in auth.users and profiles tables
    - Adds sample posts and news with proper foreign key references
  
  4. Data Integrity
    - Ensures faculties and departments exist
    - Creates proper relationships between auth users and profiles
*/
...
ON CONFLICT (id) DO UPDATE SET
  ...
  dean_email = EXCLUDED.dean_email,
  dean_phone = EXCLUDED.dean_phone;
...
BEGIN
    ...
    RAISE NOTICE 'Setup complete: % profiles, % posts, % news items', profile_count, post_count, news_count;
END $$;