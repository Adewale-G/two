/*
  # Fix User Signup and Data Type Migration

  1. User Registration Function
    - Creates automatic profile creation trigger for new users
    - Handles user metadata from signup form

  2. Data Type Fixes
    - Converts author_id columns from text to uuid
    - Fixes foreign key relationships
    - Handles dependent RLS policies properly

  3. Security Policies
    - Comprehensive RLS policies for profiles and posts
    - Proper user access controls
    - Admin access for profile management
*/
...
-- Convert author_id column from text to uuid
DO $$
BEGIN
...
BEGIN
    ...
    RAISE NOTICE 'Setup verified: % profiles, % posts, FK constraint exists: %, author_id type: %', 
                 profile_count, post_count, constraint_exists, author_id_type;
END $$;