/*
  # Fix User Signup and Data Type Issues

  1. Database Changes
    - Create trigger function to automatically create profiles for new users
    - Convert text columns to uuid where needed for proper foreign key relationships
    - Fix all foreign key constraints to use matching data types
    
  2. Security
    - Enable RLS on all tables
    - Create comprehensive policies for profiles and posts
...
CREATE OR REPLACE FUNCTION is_text_column(table_name text, column_name text)
RETURNS boolean AS $$
DECLARE
    col_type text;
BEGIN
    ...
    AND table_name = is_text_column.table_name 
    AND column_name = is_text_column.column_name;
